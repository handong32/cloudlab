...
export TBENCH_SERVER=${TBENCH_SERVER:-"192.168.1.10"}
...
function overhead
{
    ...
    ssh $TBENCH_SERVER LD_LIBRARY_PATH=/users/hand32/zygos-benchmark/silo/third-party/lz4/ perf stat -C 0-38 -e instructions,cycles,ref-cycles,cpu-clock -x, -o PerfStat.Ins.Overhead.$name /users/hand32/zygos-benchmark/servers/silotpcc-linux &
    sleep 120
    ssh $TBENCH_SERVER pkill silotpcc-linux
...
function run
{
    ...
    ssh $TBENCH_SERVER "ulimit -n 8192 && LD_LIBRARY_PATH=/users/hand32/zygos-benchmark/silo/third-party/lz4/ perf stat -C 0-38 -e instructions,cycles,ref-cycles,cpu-clock -x, -o PerfStat.Ins.$name /users/hand32/zygos-benchmark/servers/silotpcc-linux" &
    sleep 120
    taskset -c 15 ~/mutilate/mutilate --binary -s $TBENCH_SERVER --noload --agent=$AGENTS --threads=1 --keysize=19 --valuesize=2 --update=0.002 --depth=4 --measure_depth=1 --connections=10 --measure_connections=32 --measure_qps=2000 --qps=${MQPS} --time=30 > $name.out
    ssh $TBENCH_SERVER pkill silotpcc-linux
